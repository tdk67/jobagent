...
RULES_FILE = Path(__file__).parent / "data" / "role_extraction_rules.json"
...
class RoleExtractor:
    ...
    def extract_role(self, subject: str, snippet: str = "", use_llm: bool = True) -> Optional[str]:
        ...
        if use_llm and os.getenv("GEMINI_API_KEY"):
            llm_role = self.extract_role_with_llm(subject, snippet)
            if llm_role:
                return llm_role

        # 2. Fallback: Fast heuristic extraction
        lower_subj = subject.lower()
        if any(marker in lower_subj for marker in self.non_role_markers):
            return None
