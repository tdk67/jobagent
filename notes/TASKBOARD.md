...
| F3 | Gateway auth hardening (Host validation vs DNS rebinding, pair endpoint logging, Docker loopback bind) | **qa-pending** | Host-header guard on all /api/* + /a2a/* (403 invalid Host), pair logs loopback client, Dockerfile/compose bind 127.0.0.1, run_agent env overrides A2A_HOST/A2A_PORT (CLI>env>cfg); 32 tests green; evidence notes/evidence/F3-worker.md |
